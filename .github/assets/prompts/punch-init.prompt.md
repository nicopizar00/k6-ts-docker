...
1. **掃倉（scan）**：列本倉現有資產有無 —
   ...
   - 既有他工具配置：`CLAUDE.md`、`AGENTS.md`、`CODEX.md`(範圍外,記有無,不採)。
2. **安掃（security scan · 須過方續）**：掃步 1 之入,查秘鑰、私 URL、內務脈絡(中樞戒律五)。
   - **PASS** = 淨,方續喚 graphify 與繪。
   - **FAIL** = 見洩 → 阻 certify,先報洩處,人除方再掃。
3. **喚 Graphify（invoke `/graphify`）**：前置已裝,則於 Copilot Chat 喚 `/graphify .` 繪全倉圖(global graph);圖已舊則 `/graphify . --update`。圖為 certify 之據(`/punch-document` 之全倉文鏈軌)。
   - graphify **未裝** → 跳此步,圖軌記 **WARN**(不可 certify),薦人裝後再運。不硬阻採用。
4. **繪備度（map）**：對範本 `.github/assets/**` 每檔 + 圖結果,定備度閘(下)。
...
- **PASS（備）** — 真位空或一致,安掃淨,圖已繪且新,可採 / 可 certify。
- **WARN（警）** — 真位已有但異(衝突)、`<...>` 占位未填、需人調(adapt/review)、圖舊(stale)、或 **graphify 未裝**(圖軌不可 certify)。可續但須人先理。
