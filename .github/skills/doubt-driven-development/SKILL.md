...
- `code-review-and-quality` (Review) — post-hoc diff verdict; doubt-driven is
  in-flight per-decision. Use both.
- `test-driven-development` — a failing k6 check (the RED step) *is* the doubt step
  for a behavioral claim.
- `source-driven-development` — verifies framework *facts*; doubt-driven verifies
