...
```
Define → Spec → Plan → Build → Test → Review → Ship
```
...
| Test   | `punch-test`                         | `punch-test-engineer`      | `reports/state/punch-run.json` + HTML reports |
...
| Build → Test | Focused diff inside the task's allowed paths. |
| Test → Review | `reports/state/punch-run.json` with `passed: true`. |
...
- **Test fails (implementation)** → return to Plan with a corrective
  task; then Build → Test again.
- **Test fails (environment)** → human triage; probably a Doctor
  check to add.
- **Test fails (pre-existing)** → human triage; this PR is not
  blocked.
