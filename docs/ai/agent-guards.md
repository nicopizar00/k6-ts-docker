...
|---|---|---|---|---|
| `punch-builder` (coordinator) | via delegation only | n/a (delegates) | per sub-task | **may call only its registered leaves — its two engineers; no cavecrew** |
...
| `punch-code-reviewer` (Review coordinator) | no — read-only | n/a (read-only) | n/a | **optional read-only cavecrew** (`punch-cavecrew-investigator`, `punch-cavecrew-reviewer`); verdict never delegated |
| `punch-security-auditor` (Review security axis) | no — read-only | n/a (read-only) | n/a | **`punch-cavecrew-investigator` only**, optional; verdict never delegated |
| `punch-test-engineer` (Test coordinator) | **yes** — `./bin/punch` | n/a (no edit) | n/a | **`punch-cavecrew-investigator` only**, optional (read-only locate); verdict never delegated |
...
stays off. `punch-builder` lists **only its two engineers** in `agents:` — Build
never spawns cavecrew; `punch-code-reviewer` lists `punch-cavecrew-investigator` +
`punch-cavecrew-reviewer` (read-only, optional); `punch-security-auditor` + `punch-test-engineer`
list `punch-cavecrew-investigator` only (optional); `punch-release-captain` lists the three Review/Test
...
spawns them. Both remaining workers (`punch-cavecrew-investigator`,
`punch-cavecrew-reviewer`) are read-only leaves, dispatched only by the
read-only `punch-code-reviewer` / `punch-test-engineer` / `punch-security-auditor`
coordinators — Build has none to dispatch. cavecrew inherits the owning persona's **skills + allowed paths by injected
