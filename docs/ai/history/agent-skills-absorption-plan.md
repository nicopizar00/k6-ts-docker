...
> changed this pass**). Made 2026-06-16 on branch `feat/agent-skills`.
> Expands implementation order in
> [`recommended-target-ai-architecture.md`](recommended-target-ai-architecture.md)
> into scoped, reviewable tasks. Grounded in five companion review docs in
> this folder.
...
2. **Two-axis skill model** — *domain skills* (Punch's 6) + *lifecycle skills*
   (adapted upstream). Prompts stay thin, activate one of each.
...
User-directed restructure landed ahead of phased plan, changes some
assumptions below:

- **Prompts now command-mapped** to `.ai-upstream/agent-skills/commands`:
  `punch-spec`, `punch-plan`, 5× `punch-build-*`, `punch-test` (new),
  `punch-verify`, `punch-review`, `punch-ship`. **`punch-define` and
  `punch-refine` deleted**; `idea-refine` now skill invoked *within*
  `punch-spec`. Lifecycle is **6 phases** (Spec absorbs Define).
- **`punch-builder-scoped` replaced by 5 per-domain builder agents**
  (`punch-builder-orchestrator|compose|k6-http|k6-browser|data-harvest`), each
  with Copilot `tools:` scope. Agent set now 9 (4 core + 5 builders).
- **P4.0 correction (verified against VS Code docs):** prompt files use
  **`agent:`** field, *not* `mode:`. Reconciliation went toward `agent:`
  (Punch's existing convention right); 4 contract docs saying `mode:`
  were drift, now fixed. Any `mode:` recommendation elsewhere in
  these review docs **obsolete** — read as `agent:`.
- **Committed across `492b170` → `1b4b747`.** Phase 1 (drift + governance),
  builder/Define/P4.0 restructure, and **all of Phase 3** (13 lifecycle skills —
  Tier-A + P3 set) done and committed. **Phase 5 underway:** 
  `security-auditor` specialist agent added; upstream `code-reviewer` folded
  into `punch-reviewer` + `code-review-and-quality`; `test-engineer` and
  `web-performance-auditor` excluded. **Phase 6 done** — Tier-B methods folded
  ...
  recorded in `skill-registry.md`. **Phase 7 done** — `AGENTS.md` slimmed to
  pointer contract; *which skill when* discovery index added to
  `skill-registry.md` with `CLAUDE.md` pointer; upstream Node
  `validate-skills.js` folded into `punch-governance-review` (no host Node).
  **Absorption complete.** (Phase 4 — slimming phase-prompt *bodies* to
  remove prompt↔skill duplication — stays optional.)

## How "Build" works for this absorption (important)

This work edits **AI-config layer itself** (`.github/**`, `docs/ai/**`,
`AGENTS.md`, `CLAUDE.md`), not product code. So:

- The five `punch-build-*` prompts (orchestrator/compose/k6-*/data-harvest) **do
  not apply** — they for product code. AI-config edits governed by
...
- **Verify gate for AI-config tasks** = `punch-governance-review` reports
  **clean** (the "equivalent named artifact for non-test changes" that
  `operating-model.md:75` allows in place of `punch-run.json`).
- **`punch-run.json` still required** for any task that also touches product
  code (only F6.3 touches a domain skill's templates; none change runtime
  behavior — but run `./bin/punch run smoke` anyway as no-regression check).
...
`src/tests/**`, `.github/workflows/**` — unless task explicitly lists one
Allowed. Why: absorption must not change product/runtime behavior
...
Goal: clean governance baseline and skill taxonomy that *permits* lifecycle
skills. **No new skills added until this phase passes.**

| Task | Goal | Allowed | Read-only | Validation | Rollback | Checkpoint |
|---|---|---|---|---|---|---|
| **G1.1** | Register `idea-refine`; add `applies-to:`; delete the `/mnt/skills/...` line | `.github/skills/idea-refine/SKILL.md`, `docs/ai/skill-registry.md` | `agentic-workflow.instructions.md`, upstream `idea-refine` | governance-review: idea-refine has registry row + complete frontmatter | revert commit (docs only) | human confirms registry wording |
| **G1.2** (superseded) | `punch-refine` **deleted** not fixed — `idea-refine` now runs inside `punch-spec`, no standalone prompt; `prompt-registry.md` updated | `.github/prompts/punch-refine.prompt.md` (deleted), `docs/ai/prompt-registry.md` | — | governance-review clean (11 files = 11 rows) | done |
| **G1.3** | Introduce two-axis skill model: reframe cap + refine "phases-are-not-skills" | `docs/ai/skill-registry.md`, `docs/ai/operating-model.md` | conflict report C1, all instructions | governance-review clean; cap now reads "6 domain + N lifecycle" | revert commit | **human ratifies governance change** (edits Punch's own rules) |
| **G1.4** | Sync `AGENTS.md` + `copilot-instructions.md` lifecycle/asset wording to new model (no duplication) | `AGENTS.md`, `.github/copilot-instructions.md` | `operating-model.md` | governance-review: no rule restated; cross-refs resolve | revert commit | human confirms no semantic drift |

**Phase-1 exit gate:** `punch-governance-review` verdict = **"Governance is
clean"**, registries match disk, cap reframed. Diff size
...
Keep `.github/skills/`. No tasks. (Recorded so phase numbering matches
...
row, **registered in same commit** (rule the `idea-refine` precedent
...
instructions not restate them. Batched into reviewable PRs.
...
- Forbidden: standing set + **`.ai-upstream/**` read-only** (never edit
  provenance) + other skills' folders
- Validation: governance-review clean + skill's `applies-to` resolves +
  no `/mnt/`, `npm`, `jest`, `subagent_type`, or `$ARGUMENTS` leakage (grep)
- Rollback: delete skill folder + its registry row (self-contained)
- Checkpoint: human reviews *adaptation diff vs upstream* per skill
...
| **A3.3** | `debugging-and-error-recovery`, `code-review-and-quality` | Verify, Review | Code-review defers AI-config axis to `punch-governance-review`; align debugging with `punch-verify` failure classification |
...
Make prompts thin entry points activating `<lifecycle skill> + <domain
skill>` and state Punch overrides. **Procedure moves into skills, not prompts.**

> **DONE (2026-06-17).** `mode:`-vs-`agent:` contradiction resolved. VS
> Code prompt files use **`agent:`** (verified against VS Code docs), so
> prompts were already correct and the 4 contract docs saying `mode:` got
> fixed to `agent:`. Every prompt now binds to specific custom agent. P4.0
> complete.

| Task | Goal | Allowed | Read-only | Validation | Checkpoint |
|---|---|---|---|---|---|
| **P4.0** ✅ done | Reconciled prompt frontmatter to Copilot-native **`agent:`** (not `mode:`) across all prompts; each binds specific custom agent; fixed the 4 contract docs | `.github/prompts/*.prompt.md`, `docs/ai/prompt-registry.md`, `docs/ai/copilot-mode-mapping.md`, `.github/skills/punch-governance-review/SKILL.md`, `.github/instructions/agentic-workflow.instructions.md` | `copilot-setup.md`, all agents | governance-review check #1 passes (every prompt has `agent:`) | done |
...
| **P4.3** | Add new thin `punch-simplify.prompt.md` (activates `code-simplification`) + registry row | new prompt, `prompt-registry.md` | A3.4 skill | governance-review: 13 files = 13 rows | human confirms new prompt warranted |

**Phase-4 exit gate:** prompt registry in sync; no procedure duplicated between
...
| Task | Goal | Allowed | Read-only | Validation | Checkpoint |
|---|---|---|---|---|---|
| **AG5.1** | Add `security-auditor.agent.md` (adapted, Punch-scoped, read-only persona) + `AGENTS.md` row | `.github/agents/security-auditor.agent.md`, `AGENTS.md` | upstream agent, `security-and-hardening` skill | governance-review: agent listed in `AGENTS.md`; `*.agent.md` naming | human confirms persona non-duplicative |
...
| Task | Goal | Allowed | Read-only | Validation | Checkpoint |
|---|---|---|---|---|---|
| **C7.1** | Slim `AGENTS.md` to pointer contract (de-dup vs copilot-instructions/operating-model) | `AGENTS.md` | `operating-model.md` | governance-review: no restated rule | human diff review |
| **C7.2** | Build Punch skill index (adapted `using-agent-skills`) spanning domain + lifecycle skills; add CLAUDE.md skill-discovery pointer | new `.github/skills/using-punch-skills/SKILL.md` (or `skill-registry.md` section), `CLAUDE.md` | all skills | governance-review clean; index lists every registered skill | human confirms index location |
...
> C7.3 is **only task that may touch `src/punch/**`** (and only if CLI
> validator chosen). If so, follows *product-code* rules: use
> `punch-build-orchestrator`, stdlib-only, and **must** produce
> `reports/state/punch-run.json`. Else keep inside governance skill.
...
Phase 1 (G1.1→G1.4)  ──►  Phase 3 (A3.1..A3.5)  ──►  Phase 4 (P4.1..P4.3)
   ...
   └──────────────────────────┴─►  Phase 5 (AG5.x)  ─────┤
                                    Phase 6 (F6.x)   ─────┤
                                                          └─►  Phase 7 (C7.x)
```
- Phase 1 hard prerequisite for everything.
- A3.2 must land before P4.2 (Build prompts activate its skills).
- AG5.1 should follow A3.5 (security agent references security skill).
...
|---|---|---|
| Skill sprawl breaches (reframed) cap | Med | Each A3.x must name its decision/method domain in `skill-registry.md`; F6.x folds not adds |
...
| Provenance edited by mistake | Low | `.ai-upstream/**` Forbidden in every task |

## Global rollback strategy

Every task is single, self-contained commit touching only its Allowed paths
(skill folder + registry row, or one prompt + registry row). Rollback = `git
revert` that commit; no schema/contract/runtime change introduced by any
Phase 1–7 task except optional C7.3 (carries own `punch-run.json`
evidence + standard orchestrator rollback).

## Definition of done

Absorption complete when acceptance checklist in
