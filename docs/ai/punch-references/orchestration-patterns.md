...
```
/review → code-reviewer (with punch-code-review-and-quality skill) → report
```
...
### 6. Build-phase delegated execution

A slash command or prompt file **owns** the build phase and runs one coordinating builder agent. The builder may invoke bounded worker subagents — through GitHub Copilot's subagent features — to complete implementation packets. The builder remains responsible for scope control, task ordering, merging outputs, running verification, and producing the final build handoff.
...
- The user explicitly invoked `/build`.
- A spec or plan exists, or `/build auto` is allowed to generate a plan using the planning skill.
- Delegated work packets are bounded and independently verifiable.
- Subagents are one level deep only.
- Subagents do not invoke other subagents.
- The builder does not invoke lifecycle gates (`/test`, `/review`, `/ship`) on the user's behalf.
- The builder records evidence: files changed, tests run, build command, failures, commits.
...
**Cost:** one coordinator context + one engineer context + N bounded worker contexts. cavecrew workers report in compressed `wenyan-ultra`, so their tool-results stay small; the builder spends its context on merge + verification, not on re-deriving locations the investigator already mapped.
...
**Not this:** Pattern 6 (Build-phase delegated execution). A command-owned coordinator for an *already-selected* phase that owns real work and respects depth ≤ 1 is endorsed, not a router persona. The line: a router *chooses the phase*; a coordinator *executes the chosen one*.
