...
| [`punch-document`](../../.github/prompts/punch-document.prompt.md) | Documentate (recurring maintenance) | Ask / Agent | `punch-ai-governance` | Remediate doc debt + AI-artifact lifecycle in waves: map with `/graphify`, verify inherited artifacts (untrusted by default), then keep / merge / compact / convert / promote / archive / delete / review. Sole Graphify write owner — build/`--update`/regenerate run only here. |
