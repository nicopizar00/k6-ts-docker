...
Punch have **nine** prompts — one per lifecycle phase (Spec, Plan, Build, Verify,
Review, Ship), plus `punch-test` (TDD/Prove-It companion to Verify),
`punch-document` (recurring doc-reconciliation phase, orthogonal to linear
...
| [`punch-test`](../../.github/prompts/punch-test.prompt.md) | Test (Verify companion) | Agent | `punch-verifier` | Prove change RED→GREEN at k6 check/threshold level via `./bin/punch`. |
| [`punch-verify`](../../.github/prompts/punch-verify.prompt.md) | Verify | Agent / Ask | `punch-verifier` | Build complete; need `reports/state/punch-run.json` evidence. |
| [`punch-review`](../../.github/prompts/punch-review.prompt.md) | Review | Ask | `punch-reviewer` | Verify passed; audit diff before Ship. |
...
type multiply maintenance, add no value. (`punch-test` thin Verify companion, not
separate phase.)
