...
| [`punch-init`](../../.github/prompts/punch-init.prompt.md) | Init (bootstrap, on-demand) | Ask (read-only asset sweep) | `punch-ai-governance` (enforced) | Certify the GitHub Copilot asset set (prompts, agents, skills, instructions) is present, `punch-`prefixed, and Copilot-compatible, plus an informational capability check for the optional AI-Ingest Caveman/cavecrew vendor skills. Reports PASS/WARN/BLOCKED; hands reconciliation to `/punch-document`. No runtime, no Python. |
